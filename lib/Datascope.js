...
var Datascope = React.createClass({
    ...
    render: function render() {
        ...
        var datascopeProps = _.assign({}, _.pick(this.props, ['data', 'schema', 'query']), { fields: fields, orderedFields: orderedFields });
...
function initFields(definedFields, schema) {
    ...
    _.each(definedFields, function (definedField, fieldKey) {
        if (!fieldKey in fields) return; // todo allow additional fields with key prop
        ...
        _.assign(fields[fieldKey], fieldProps);
    });
...
function fieldsFromSchema(schema) {
    ...
    return _(schema.items.properties).map(function (propSchema, key) {
        return [key, {
            ...
            key: key,
            name: key,
            renderer: propSchema.type && propSchema.type in fieldDefaults.renderers ? fieldDefaults.renderers[propSchema.type] : function (v) {
                return v + '';
            }
        }];
    }).object().value();
