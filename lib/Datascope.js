...
function fieldsFromSchema(schema) {
    ...
    return _(schema.items.properties).map(function (propSchema, key) {
        return [key, {
            ...
            key: key,
            name: key,
            renderer: propSchema.type && propSchema.type in fieldDefaults.renderers ? fieldDefaults.renderers[propSchema.type] : propSchema.oneOf && _.every(propSchema.oneOf, function (s) {
                return s.title && s['enum'] && s['enum'].length == 1;
            }) ? fieldDefaults.renderers.oneOf : function (v) {
                return v + '';
            }
        }];
    }).object().value();
